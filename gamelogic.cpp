...
{
    DatabaseManager::instance().saveUserScore(
        userId, currentLanguage, correctAnswersCount, 
        currentQuizQuestions.size(), currentScore
    );
}
...
void GameLogic::loadLanguageData() { /* Opcionális */ }
...
void GameLogic::refreshQuestionPool(Category cat, Difficulty diff)
{
    ...
    if (!currentQuizQuestions.isEmpty()) currentQuestionIndex = 0;
...
{
    if (currentQuestionIndex < currentQuizQuestions.size()) currentQuestionIndex++;
}
