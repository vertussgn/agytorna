...
QVector<Question> DatabaseManager::loadQuestions(Language language, Category category, Difficulty difficulty, int limit)
{
    ...
    q.prepare("SELECT id, question_text, word, answer_a, answer_b, answer_c, answer_d, correct_answer, points "
              "FROM questions "
    ...
    while (q.next()) {
        ...
        qu.answers = {q.value(3).toString(), q.value(4).toString(), q.value(5).toString(), q.value(6).toString()};
...
bool DatabaseManager::saveUserScore(int userId, Language language, int correctAnswers, int totalQuestions, int totalPoints)
{
    ...
    QSqlQuery q(m_db);
    q.prepare("INSERT INTO user_scores (user_id, language, correct_answers, total_questions, total_points) VALUES (:uid, :lang, :ca, :tq, :tp)");
...
QStringList DatabaseManager::loadHighScores(int limit)
{
    ...
    QSqlQuery q(m_db);
    q.prepare("SELECT u.username, SUM(s.total_points) as grand_total FROM user_scores s JOIN users u ON s.user_id = u.id GROUP BY u.id ORDER BY grand_total DESC LIMIT :limit");
    q.bindValue(":limit", limit);
...
PlayerStats DatabaseManager::getPlayerStatistics(int userId)
{
    ...
    QSqlQuery q(m_db);
    q.prepare("SELECT SUM(total_points), SUM(total_questions), SUM(correct_answers), MAX(played_at) FROM user_scores WHERE user_id = :uid");
    q.bindValue(":uid", userId);
