...
void TestRunner::testDatabaseConnection()
{
    ...
    if (isConnected) {
        ...
        QaManager::logSystem(
            "[TESZT KRITIKUS HIBA] Adatbázis nem elérhető! A további tesztek bukhatnak.", true);
    }
}
...
void TestRunner::testUserDatabaseOperations()
{
    ...
    bool saveSuccess = DatabaseManager::instance().saveUserScore(userId,
                                                                 Language::English,
                                                                 5,
                                                                 10,
                                                                 50);
    ...
    if (stats.totalScore >= 50 && stats.totalQuestionsAnswered >= 10) {
        ...
        QaManager::logSystem("[TESZT HIBA] A visszaolvasott statisztika nem egyezik a mentettel!",
                             true);
    }
}
...
{
    QVector<Question> questions = DatabaseManager::instance().loadQuestions(Language::English,
                                                                            Category::Vocabulary,
                                                                            Difficulty::Beginner,
                                                                            5);
    ...
    if (questions.isEmpty()) {
        QaManager::logSystem(
            "[TESZT FIGYELMEZTETÉS] Üres az adatbázis kérdés táblája (vagy szűrési hiba).", true);
        return;
    }

    int errorCount = 0;
    for (const Question &q : questions) {
        if (q.questionText.isEmpty() || q.answers.size() != 4 || q.correctAnswer < 0
            || q.correctAnswer > 3) {
            errorCount++;
    ...
    if (errorCount == 0) {
        QaManager::logSystem(QString("[TESZT OK] %1 kérdés ellenőrizve: Szerkezetileg mind helyes.")
                                 .arg(questions.size()));
    }
}
...
void TestRunner::testGameLogicSimulation()
{
    ...
    if (logic.getCurrentQuestionIndex() == 1 || logic.isQuizFinished()) {
        ...
        QaManager::logSystem("[TESZT HIBA] nextQuestion() nem léptette az indexet megfelelően.",
                             true);
    }
}
