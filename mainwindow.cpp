...
void MainWindow::showResults()
{
    ...
    QString scoreText = QString("%1 / %2").arg(correctAnswers).arg(questions.size());
    ui->resultLabel->setText(scoreText);

    double percentage = (questions.size() > 0) ? (correctAnswers * 100.0 / questions.size()) : 0.0;
...
{
    selectedCategory = Category::Vocabulary;
    ui->vocabularyButton->setStyleSheet("QPushButton { background-color: #e74c3c; color: white; }");
}

void MainWindow::onGrammarSelected()
{
    selectedCategory = Category::Grammar;
    ui->grammarButton->setStyleSheet("QPushButton { background-color: #e74c3c; color: white; }");
}

void MainWindow::onSentencesSelected()
{
    selectedCategory = Category::Sentences;
    ui->sentencesButton->setStyleSheet("QPushButton { background-color: #e74c3c; color: white; }");
}

void MainWindow::onListeningSelected()
{
    selectedCategory = Category::Listening;
    ui->listeningButton->setStyleSheet("QPushButton { background-color: #e74c3c; color: white; }");
}

void MainWindow::onBeginnerSelected()
{
    selectedDifficulty = Difficulty::Beginner;
    ui->beginnerButton->setStyleSheet("QPushButton { background-color: #f39c12; color: white; }");
}
...
void MainWindow::onAdvancedSelected()
{
    ...
    ui->advancedButton->setStyleSheet("QPushButton { background-color: #f39c12; color: white; }");
}
...
void MainWindow::setupConnections()
{
    ...
    connect(ui->backFromStatsButton, &QPushButton::clicked, this, &MainWindow::onBackToMenu);
...
void MainWindow::loadQuestions()
{
    ...
    // ========================================================================
    questions.append({"Mit jelent ez a szó magyarul?",
                      "apple",
    ...
    // ========================================================================
    questions.append({"Melyik a helyes mondat?",
                      "I ___ a student.",
                      {"am", "is", "are", "be"},
                      0,
                      10,
    ...
    // ========================================================================
    questions.append({"Fordítsd le magyarra:",
                      "Hello, how are you?",
...
{
    QString counterText = QString("Kérdés %1/%2").arg(currentQuestionIndex + 1).arg(questions.size());
...
void MainWindow::filterQuestions()
{
    ...
    for (const Question &q : allQuestions) {
        if (q.language == selectedLanguage && q.category == selectedCategory
